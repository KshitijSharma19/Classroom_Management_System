...
import { 
  ...
  User as UserIcon, Bell, Settings, LogOut, Menu, X, 
...
export default function TeacherLayout() {
  ...
  const navItems = [
    { name: 'Dashboard', path: '/teacher', icon: Home, end: true },
    { name: 'My Classes', path: '/teacher/classes', icon: BookOpen },
    { name: 'Students', path: '/teacher/students', icon: UserIcon },
    { name: 'Assignments', path: '/teacher/assignments', icon: CheckSquare },
    { name: 'Notice Board', path: '/teacher/notices', icon: Bell },
  ];
    ...
    <div className="flex h-screen bg-[#FDF8F5] dark:bg-gray-950 font-sans text-gray-900 dark:text-gray-100 transition-colors duration-300 overflow-hidden">
      ...
      `}>
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-4 space-y-2">
          <div className="text-xs font-bold tracking-wider text-[#935F53] uppercase mb-4 px-2">Teacher Modules</div>
        ...
        <header className="h-20 bg-[#FDF8F5]/80 dark:bg-gray-950/80 backdrop-blur-md border-b border-[#EBE0C8] dark:border-gray-800 flex items-center justify-between px-4 sm:px-8 z-30">
          <div className="flex items-center space-x-4">
            ...
            <div className="hidden md:flex flex-col">
              <span className="font-bold text-[#462F2D] dark:text-white">Teacher Portal</span>
                ...
                <>
                  ...
                  <div className="absolute right-0 mt-2 w-72 bg-white dark:bg-gray-900 border border-[#EBE0C8] dark:border-gray-800 rounded-2xl shadow-xl z-50 overflow-hidden">
                    ...
                    <div className="p-4 border-b border-[#EBE0C8] dark:border-gray-800">
                      <p className="text-sm font-bold text-[#462F2D] dark:text-white mb-1">New Assignments Submitted</p>
                      <p className="text-xs text-[#735A52] dark:text-gray-400">12 students submitted their work.</p>
                    </div>
                    <button onClick={() => { setShowNotifications(false); navigate('/teacher/notices'); }} className="w-full p-3 text-sm font-bold text-[#935F53] dark:text-indigo-400 hover:bg-[#F5EEDC] dark:hover:bg-gray-800 transition-colors">
                      View all notices
                    </button>
                  </div>
                </>
            ...
            <div className="relative">
              ...
              >
                ...
                <div className="hidden sm:block text-left">
                  <p className="text-sm font-bold text-[#462F2D] dark:text-white leading-tight">{user?.name || 'Teacher'}</p>
                  <p className="text-xs text-[#735A52] dark:text-gray-400 capitalize">{user?.role || 'Teacher'}</p>
                </div>
              </div>
              ...
              {showProfileMenu && (
                <>
                  ...
                  <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-900 border border-[#EBE0C8] dark:border-gray-800 rounded-2xl shadow-xl z-50 overflow-hidden">
                    <button onClick={() => { setShowProfileMenu(false); navigate('/teacher/settings'); }} className="w-full px-4 py-3 text-left flex items-center space-x-3 text-sm font-bold text-[#735A52] dark:text-gray-300 hover:bg-[#F5EEDC] dark:hover:bg-gray-800 transition-colors border-b border-[#EBE0C8] dark:border-gray-800">
